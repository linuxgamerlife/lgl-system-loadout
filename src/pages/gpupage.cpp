...
void GpuPage::initializePage()
{
    ...
    toolbar->addStretch();
        ...
        const QList<std::tuple<QString,QString,QString>> amdItems = {
            ...
            {"mesa_vdpau",    "mesa-vdpau-drivers",
             "VDPAU driver for AMD via Mesa - used by some older applications and VLC."},
    ...
    // Run AMD install checks concurrently
    ...
    for (auto &c : _checks) {
        if (c.first == "mesa_vdpau")
            c.second = []{ return isDnfInstalled("mesa-vdpau-drivers") || isDnfInstalled("mesa-vdpau-drivers-freeworld"); };
    }
