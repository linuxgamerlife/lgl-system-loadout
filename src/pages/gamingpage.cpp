...
void GamingPage::initializePage()
{
    ...
    const QList<Item> dnfItems = {
        ...
        {"wine",         "Wine",         "Compatibility layer for running Windows applications and games on Linux."},
        {"protontricks", "Protontricks", "Winetricks wrapper for managing Proton-managed Steam games."},
        {"winetricks",   "Winetricks",   "Helper script for installing Windows libraries and components into Wine prefixes."},
        ...
        {"gamemode",     "gamemode",     "Automatically applies CPU/GPU performance optimisations when a game launches."},
        {"gamescope",    "gamescope",    "Valve's micro-compositor for games. Enables resolution scaling and FSR upscaling."},
    };
    ...
    for (const auto &it : QList<Item>{dnfItems[2], dnfItems[3], dnfItems[4]}) {
        auto *cb = makeItemRow(inner, layout, it.label, false);
        layout->addWidget(makeDescLabel(inner, it.desc)); layout->addSpacing(2);
        m_boxes[it.key] = cb;
    }
    ...
    for (const auto &it : QList<Item>{dnfItems[5], dnfItems[6], dnfItems[7], dnfItems[8], dnfItems[9]}) {
        auto *cb = makeItemRow(inner, layout, it.label, false);
        layout->addWidget(makeDescLabel(inner, it.desc)); layout->addSpacing(2);
        m_boxes[it.key] = cb;
    }
